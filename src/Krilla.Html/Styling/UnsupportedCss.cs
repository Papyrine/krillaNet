...
static class UnsupportedCss
{
    ...
    static void Radius(
        ...
        Action<HtmlDiagnostic> sink)
    {
        // Honoured for the background always, and for a border wherever its frame can be cut to a
        // rounded ring: one ring where every edge agrees, and a mitred trapezium or an inline mitre
        // sector clipped to that same ring where they do not. The two returns below are those.
        if (!style.HasBorder)
        {
            return;
        }
        ...
        // A mitred edge is clipped to its own trapezium and the rounded ring drawn through it, so
        // two colours still hand over on the corner diagonal and the corner is curved - whatever
        // the edges disagree about. What is left is the patterned styles, whose dashes and dots run
        // along a straight centre line past the corner and are not mitred at all.
        if (!HasPatternedEdge(style))
        {
            return;
        }
