...
- **`.github/workflows/build.yml`** — builds and tests the managed side on Linux, Windows and macOS. Deliberately lets `dotnet build` drive cargo through `KrillaBuildNative` rather than invoking it directly, so the MSBuild wiring itself gets exercised on all three platforms. Also calls `native.yml`, packs all nine natives into one package (the `nupkgs` artifact), runs `IntegrationTests` against the real nupkg on four runners plus Alpine and Ubuntu 22.04 containers, and on a tag publishes via nuget.org Trusted Publishing (OIDC) from a `publish` job in the `nuget` environment. Uploads `*.received.*` on failure, without which a snapshot mismatch on a platform nobody has locally is close to undebuggable.
...
- **`.github/workflows/native.yml`** — the nine-RID cross-compilation matrix, called by `build.yml` (also `workflow_dispatch`). Each leg verifies its own output; eight do it statically, and `browser-wasm` cannot — being usable is a property of the LINK rather than of the file — so that leg builds a C driver against the archive and runs it under node. See the header comment for why each check exists.
...
- **The RID table is duplicated in three places** and all three must stay in sync: `src/Krilla.Native.props` (the MSBuild table), `.github/workflows/native.yml` (the build matrix), and `readme.md` (documented support). `build.yml`'s package-contents check enumerates them a fourth time, and being a list of strings rather than a table it is the one that drifts silently — a missing RID there means the check passes over exactly the hole it exists to find.
...
- **`build.yml` produces a host-RID-only package** and must never publish it. The publishable one is the `pack` job's `nupkgs` artifact in the same workflow, built from the full nine-RID matrix.
...
- **MarkdownSnippets must stay out of `rust/target`.** Two projects reference MarkdownSnippets, their scans run concurrently during a solution build, and both would open `rust/target/.cargo-artifact-lock` — which fails the build with "another process has locked a portion of the file". Since 28.4.4 MarkdownSnippets skips any directory holding a `CACHEDIR.TAG`, which cargo writes into `target`, so no `mdsnippets.json` exclusion is needed. It only reproduces once the native has been built, so a clean checkout will not show a regression.
