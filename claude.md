...
Known and still unaudited: `vertical-align` on an inline box (only the table-cell case is honoured or reported, and `sub`/`sup` take theirs from the default stylesheet, so a report would need the exemption `hr` has), `outline`, `object-fit`, and a tab under `pre`, which reaches the shaper as a character rather than advancing to a tab stop. `hyphens` came off that list with the line-breaking work and reports only `auto` and `all` — `manual` is what an author writes to DISABLE automatic hyphenation, and reporting it would fire on documents that convert perfectly.
