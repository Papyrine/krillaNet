...
## Measured, and not yet implemented: border-collapse

The numbers below came out of Chrome and are recorded so the work starts from data. There is no
corpus scenario, because a scenario without an implementation can only be committed with a
knowingly wrong baseline.

- **Each cell's border box is inset by HALF the resolved border on each edge**, and the table's border box extends half the resolved outer border beyond the outermost cells. A 2px uniform collapse on 80px cells with 6px padding gives 94px cells inside a 190px table.
- **The widest border wins a shared edge outright.** A 6px cell against a 2px neighbour puts 3px either side of their common line; the 2px cell comes out 96px wide against its neighbour's 98px.
- **A table's own border competes on the same terms** and wins the outer edges when it is wider: a 4px table border against 2px cells puts the cells 2px inside the table box.
- **`border-spacing` stops applying entirely**, and the row box spans the cells rather than sitting between them.
- The hard part is not the arithmetic but where it lands: the cell surrounds feed the column algorithm, so the halved widths have to be resolved BEFORE it runs, and painting has to draw each grid line once at full width rather than letting two cells each paint their half — two abutting half-borders seam at any odd width.
...
`border-collapse` is measured and NOT implemented — the numbers are in the trap list below, so whoever takes it starts from data rather than from the specification. Known and still unaudited: `object-position`, `vertical-align` on an inline box (only the table-cell case is honoured or reported, and `sub`/`sup` take theirs from the default stylesheet, so a report would need the exemption `hr` has), `outline`, `object-fit`, and a tab under `pre`, which reaches the shaper as a character rather than advancing to a tab stop. `hyphens` came off that list with the line-breaking work and reports only `auto` and `all` — `manual` is what an author writes to DISABLE automatic hyphenation, and reporting it would fire on documents that convert perfectly.
